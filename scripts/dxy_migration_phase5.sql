...
-- - 5m data: Keep reading from derived_data_bars only (no change needed)
...
BEGIN
  ...
  -- MIGRATION CHANGE: Removed UNION ALL
...
-- ============================================================================
-- Update catchup_aggregation_range max source check for 1m
-- ============================================================================
...
DECLARE
  ...
  v_src_tf text;
BEGIN
  -- CRITICAL: Guard against NULL cursor
  ...
  -- MIGRATION CHANGE: Get max source timestamp based on timeframe
  IF v_src_tf = '1m' THEN
    -- For 1m source, only check data_bars (DXY now there)
    SELECT MAX(ts_utc) INTO v_max_source_ts
    FROM data_bars
    WHERE canonical_symbol = p_symbol AND timeframe = v_src_tf;
  ELSE
    -- For 5m+ source, check derived_data_bars only
    SELECT MAX(ts_utc) INTO v_max_source_ts
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '==========================================================';
  RAISE NOTICE 'DXY MIGRATION - PHASE 5 COMPLETE';
  RAISE NOTICE '==========================================================';
  RAISE NOTICE '';
  RAISE NOTICE 'Changes applied:';
  RAISE NOTICE '  1. aggregate_1m_to_5m_window: Now reads ONLY from data_bars';
  RAISE NOTICE '     (DXY 1m is now in data_bars, not derived_data_bars)';
  RAISE NOTICE '';
  RAISE NOTICE '  2. catchup_aggregation_range: Smart max source check';
  RAISE NOTICE '     - For 1m source: checks data_bars only';
  RAISE NOTICE '     - For 5m+ source: checks derived_data_bars only';
  RAISE NOTICE '';
  RAISE NOTICE 'Next step: Test aggregation';
  RAISE NOTICE '  SELECT aggregate_1m_to_5m_window(''DXY'', NOW() - INTERVAL ''5 minutes'', NOW(), 1);';
  RAISE NOTICE '';
END $$;
